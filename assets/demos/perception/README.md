...
| 按键 | 作用 |
|---|---|
| `→` / `空格` / `Enter` / `PageDown` / `n` | 下一状态（也支持点击屏幕空白处；盲点、贴标签两幕除外） |
| `←` / `PageUp` / `p` | 上一状态 |
| `↑` / `↓` | 上一 / 下一场景 |
| `1`–`9` | 跳到第 1–9 个场景 |
| `0` | 跳到「收束」那一幕 |
| `Home` | 回到开场 |
| `X`（或翻页笔的黑屏键 `.`） | 黑屏（临时遮挡，再按恢复） |
| `F` | 全屏 |
| `D` | 调试面板（也可以直接用 `?debug=1` 打开） |
| `?` / `/` | 显示快捷键提示 |
| `R` | 只在「树倒悖论」那一幕有效：清空举手票数 |
| `Esc` | 依次关掉：黑屏 → 调试面板 → 退出全屏 |
...
| 3 | `aphantasia` | 心中世界每人不同 | 5 | 想象 → 举手 → 对比 → 三句话 |
| 4 | `mind` | 心在哪里 | 5 | 心脏中心说 → 数据 → 心理学凭什么独立 |
| 5 | `brain` | 大脑探秘 | 4 | 展示 → 脑区 → 幻肢痛 |
| 6 | `stratton` | 自我进化 · 颠倒的世界 | 7 | 先猜一猜 → 世界颠倒 → 2025 证据 |
| 7 | `eagleman` | 自我进化 · 换一条通道 | 7 | 先猜一猜 → 背心 |
| 8 | `illusion` | 如何研究脑 · 主观报告不可信 | 3 | 知道真相也消除不了 |
| 9 | `eeg` | 如何研究脑 · 从标本到信号 | 11 | 爱因斯坦 → 人脑库 → 先问「你会怎么测」→ 脑电 |
| 10 | `bci` | 如何改变脑 · 从读到写 | 3 | 读/写两条通路 → 四种接口 → 边界 |
| 11 | `closing` | 收束 | 3 | |
| 12 | `awe` | 回响 | 4 | |
| 13 | `curtain` | 下课 | 1 | |
...
1. **斯特拉顿和 Eagleman 是同一件事**（换掉输入 → 大脑重新接线），
   所以放一起，都归「**大脑自己会改**」；脑机接口才是「**我们直接改它**」。
   这两者不能混。
2. **「如何研究」排在「如何改变」之前** —— 先能看，才能改。
...
- **`tree` 里有两次统计**（1.1 和 1.3），两次共用同一组数字，**第二次为准**。
  第 13 幕（`awe`）的回扣句会自动判断：**只有「不会响」占多数时才说「我们当时说：不会响」**，
  否则改说「我们当时投出来的，是「会响」」。所以你怎么投都不会露馅，
  但**两次都要真投**，回扣才有东西。
- **`aphantasia.0 → 1`** 是「闭眼想象 → 睁眼举手」。举完手环顾全场的那一下
  是这一幕的重点，别急着按下一张。
- **`brain.3` 只是一句过渡**（从「感觉在大脑」推到「线路能不能改」），带过就行，别停。
...
- **`closing` 原来那一屏「求索卡」已经去掉**（要印卡片 + 两个二维码）。
  收尾话现在在最后一幕（`curtain`）的幕布上：「谢谢大家，欢迎提问。」
...
8.9 再回应那句「学心理的是不是都有病」：心理学的对象是人自己，
...
### ⚠️ 前进和后退必须对称（`core.js` 的闸门）

**症状**（用户报的）：「前进时候看到的组件出现顺序和回退的时候不一样」。

**原因**：`next()` 有闸门（一次露一行），但 `prev()` 原来**没有**。
于是按 4 下才过得去的一屏，回退 1 下就跳过去了 —— 而且已经露出的行全部丢掉。

**修法**（`core.js`）：
1. 加 `prevGate`，`prev()` 和 `next()` 对称地先查闸门；
2. `stepPos["场景:状态"]` **记住每一屏露到第几行**，来回走时按原样恢复 ——
   往回走到某一屏，看到的是你离开它时的样子，而不是从头再来。

```js
function sync() {
  stepPos[key] = i;
  ctx.holdNext(i < items.length ? advance : null);  // 还有没露的 → 拦前进
  ctx.holdPrev(i > 0 ? retreat : null);             // 露过至少一行 → 拦后退
}
```

**验证脚本**：`/tmp/ordercheck.js` —— 逐屏记录「屏幕上真正看得见的文字」，
前进 12 步、回退 12 步，然后逐条比对两个序列。
修之前 11 处不对称，修之后只剩 1 处，而且那一处是 `tree.0` 用自动错开进场、
测试抓拍早了造成的，不是跳页。
...
紧接着 `eeg.1` 才是人脑库（七百多例）—— 顺序是「一个不够 → 所以要很多个」。
...
| 位置 | 引文 | 出处 |
|---|---|---|
| `tree` 1.4 | You're not perceiving what's out there… | Eagleman, *Incognito* |
| `blindspot` 2.1 | Instead of reality being passively recorded… | Eagleman, *Incognito* |
| `illusion` 8.2 | So the first lesson about trusting your senses is: don't. | Eagleman, *Incognito* |
| `stratton` 6.5 | The brain is a dynamic system… | Eagleman, *Livewired* |
| `closing` 11.2 | If the human brain were so simple that we could understand it… | Emerson M. Pugh（约 1938，1977 年首次发表；Lyall Watson 1979 年传播开来） |
