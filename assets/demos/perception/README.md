...
- 3D 大脑改成了程序化生成的内联 SVG 侧视图 —— 原设计写的是 Three.js + GLB，
  ...
  画法值得记一笔：**脑回是一条条「流线」**（8 个不同方向的正弦叠成方向场，
  从几百个起点沿场走短线），不是一排行波 —— 行波画出来永远像等高线／木纹。
  流线要短、场要有高频成分，否则整个场的大漩涡会透出来，变成带结疤的木纹。
  底色是一层径向渐变给体积，脑叶本身不填色、只负责接指针（`pointer-events:all`），
  脑回画在脑叶**之上** —— 这样高亮时蓝色是「透过褶皱透出来」的，而不是一块贴纸。
