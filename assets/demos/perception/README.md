...
於潜中学心理学科普讲座的交互式演示。13 个场景、58 个状态，
...
| 1 | `tree` | 树倒悖论 | 7 | 两问 → 一次统计 → 声音通路 → 二次统计 → 拆解 |
...
| 7 | `illusion` | 大小错觉 | 3 | 三拍：猜 → 证明一样大 → 知道了再看还是错 |
| 8 | `eeg` | 脑电波演示 | 9 | 暗色场景；含科学精神与人脑库 |
...
> 窗外那棵树会不会响？这件事会因为你在不在场改变吗？
> → 无人的森林里那棵树会不会响？（**第一次统计**）
> → 声音到底是怎么发生的：声波→耳廓→耳道→鼓膜→听小骨→耳蜗→听神经→大脑
> → 那现在再投一次（**第二次统计，以这次为准**）
> → 声波在森林里，响在大脑里
> → 大脑会替你补上不存在的东西（盲点）
> → 不同的大脑补出来的不一样（心盲症）
