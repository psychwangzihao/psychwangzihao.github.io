...
- 3D 大脑改成了程序化生成的内联 SVG 侧视图 —— 原设计写的是 Three.js + GLB，
  ...
  画法值得记一笔 —— **脑回是一个平滑标量场的等值线**（marching squares），
  不是线条也不是流线。中途试错试了三轮，都是真踩过的坑：

  1. **一排行波** → 一道道等宽平行线，看起来像等高线 / 木纹。
  2. **沿方向场积分出的流线** → 频率低了是一把顺毛（全朝一个方向），
     频率高了会急拐弯，变成一团树根。中间没有可用的一档。
  3. **等值线，但场里只有 6 条波长齐整的正弦** → 峰多鞍点少，
     等值线围着每个峰绕成一圈圈同心环，又变回等高线图。
  4. **等值线 + 18 条方向、波长、权重都散开的正弦** → 峰和鞍点数量相当，
     等值线连成长长短短、到处拐弯的迷宫。这才是 🧠 那个脑子看起来像脑的原因。

  另外两条经验：等值线的**层数别写死**，要按数值估计的梯度把褶皱间距
  钉在一个目标值上（否则一改波长就糊成一片或稀成等高线）；
  褶皱要有厚度，就得**画两遍**——先画一遍偏右下、偏亮的当受光的那面墙，
  再把深色压在原位，一个亮边加一道深槽就立起来了。

  底色是一层径向渐变给体积；脑叶本身不填色、只负责接指针
  （`pointer-events:all`，`fill:none` 的元素默认不响应指针）；
