...
(function () {
  ...
  function makeCtx() {
    ...
    var ctx = {
      ...
      soon: function (fn, delay) {
        /* ⚠️ 里面的 rAF 也必须登记进 kill —— 只登记 setTimeout 的话，
           定时器在切场景前一瞬触发、把 rAF 排进队列，这一帧就会在
           场景已经换掉之后才跑。而 ctx.q() 解析的是**当前**场景的
           元素（curEl 是模块级变量，不是闭包副本），于是拿到 null，
           报 "Cannot read properties of null (reading 'classList')"。
           原版就有这个漏，快速连翻时能连着报好几条。 */
        ctx.after(delay || 30, function () {
          var id = requestAnimationFrame(fn);
          kill.push(function () { cancelAnimationFrame(id); });
        });
      },
  ...
  function updateChrome() {
    ...
    /* 导航圆点。场景可以带 part 字段（见 PERCEPTION.scene）——
       连续几个同 part 的场景会在圆点条上被归成一组，组前标出名字。
       这样「感知的多样性」是六幕，但在导航上是**一个块**。 */
    var html = '', lastPart = '';
    for (var i = 0; i < SCENES.length; i++) {
      var p = SCENES[i].part || '';
      if (p !== lastPart) {
        /* 换 Part 就断开。没有名字的那些（过渡幕）也要断 ——
           不断的话它们会被吸进相邻的一组，圆点分不清是谁的。 */
        html += p ? '<span class="part">' + p + '</span>'
    ...
    /* 读数里不再重复 Part 名字 —— 圆点条上那一组已经标了 */
    html += '<span class="lbl">' + (si + 1) + '/' + SCENES.length + ' · ' +
            (st + 1) + '/' + SCENES[si].states.length + '</span>';
